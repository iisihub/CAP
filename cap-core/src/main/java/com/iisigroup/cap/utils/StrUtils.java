...
public class StrUtils {
	...
	public static String[] splitFixedLength(String str, final int len) {
		...
		while (idx < blen) {
			String tmp = new String(bmsg, idx, idx + len < blen ? len : blen
					- idx);
			tempList.add(tmp);
			idx += len;
	...
	public static int getEntityFileldLegth(Class clazz, String filedName,
			int defValue) {
		try {
			return clazz.getDeclaredField(filedName).getAnnotation(Column.class)
					.length();
		} catch (Exception e) {
			e.printStackTrace();
			return defValue;
