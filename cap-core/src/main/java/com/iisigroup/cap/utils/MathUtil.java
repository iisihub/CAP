...
public class MathUtil implements Constants {
	...
	public static String subtract(String first, String second, int scale) {
		...
		BigDecimal f = getBigDecimal(first);// new BigDecimal(first);
		BigDecimal s = getBigDecimal(second);// new BigDecimal(second);
		BigDecimal result = f.subtract(s)
	...
	public static String divide(String first, String second) {
		...
		BigDecimal f = getBigDecimal(first);// new BigDecimal(first);
		BigDecimal s = getBigDecimal(second);// new BigDecimal(second);
		if (s.compareTo(B0)==0) return "0";
		int fs = f.scale();
		int ss = s.scale();
		int max = fs > ss ? fs : ss;
		BigDecimal result = f.setScale(max, DEFAULT_DIVIDE_ROUNDING_MODE[0])
				.divide(s, DEFAULT_DIVIDE_ROUNDING_MODE[1]).setScale(max,
						DEFAULT_DIVIDE_ROUNDING_MODE[2]);
	...
	public static String divide(String first, String second, int scale) {
		...
		BigDecimal f = getBigDecimal(first);// new BigDecimal(first);
		BigDecimal s = getBigDecimal(second);// new BigDecimal(second);
		BigDecimal result = f.setScale(scale, DEFAULT_DIVIDE_ROUNDING_MODE[0])
				.divide(s, DEFAULT_DIVIDE_ROUNDING_MODE[1]).setScale(scale,
						DEFAULT_DIVIDE_ROUNDING_MODE[2]);
		return bigDecimalToString(result);
	}
	...
	public static int compare(String first, String second) {
		try {
			first = first == null || first.length() == 0 ? S0 : first.trim();
			second = second == null || second.length() == 0 ? S0 : second
					.trim();
	 ...
	 * @param sampleValue sampleValue
	 * @param sampleMean sampleMean
	 * @param sampleSTD sampleSTD
	 * @param mean mean
	 * @param std std
	 * @param scale scale
	 * @param rm rm
