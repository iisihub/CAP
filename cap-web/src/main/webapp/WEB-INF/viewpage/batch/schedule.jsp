...
<title><spring:message code="tit.schedule"><!--排程設定--></spring:message></title>
</head>
<body>
	<script>
		...
		<div id="editDialog" class="hide" title="<spring:message code="jobs.editDialog" />" >
			...
			<table class="row-data">
				...
				<tr>
					...
					<td><select id="jobId" name="jobId">
						<c:forEach items="${batchJob}" var="job">
							<option value="<c:out value="${job.jobId}"/>"><c:out value="${job.jobId}"/>-<c:out value="${job.jobDesc}"/></option>
						</c:forEach>
					</select></td>
				</tr>
				<tr>
					...
					<td><label><input type="radio" id="isEnabled" name="isEnabled" value="Y" /><spring:message code="js.yes"/></label>
						<label><input type="radio" id="isEnabled" name="isEnabled" value="N" checked="checked"/><spring:message code="js.no"/></label></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><label><input type="radio" id="schType" name="schType" value="C" checked="checked"/><spring:message code="sch.schType.C"/></label>
						<label><input type="radio" id="schType" name="schType" value="T" /><spring:message code="sch.schType.T"/></label></td>
				</tr>
				<tr class="schTypeC show">
					<th><spring:message code="sch.cronExpression">
							<!--執行週期-->
						</spring:message></th>
					<td><input type="text" id="cronExpression" name="cronExpression" size="30" maxlength="120"/>(<a target="_blank" href="http://docs.oracle.com/cd/E12058_01/doc/doc.1014/e12030/cron_expressions.htm"><spring:message code="js.explain"/></a>)</td>
				</tr>
				<tr class="schTypeC show">
				...
				<tr>
					...
					<td><label><input type="radio" id="isNotify" name="isNotify" value="Y" /><spring:message code="js.yes"/></label>
				...
				<tr class="notify hide">
					...
					<td><textarea id="notifyTo" name="notifyTo" cols="70" rows="2" maxlength="200" class="validate[funcCall[maxUTF8[200]]]" ></textarea><br/>
						<spring:message code="js.CommaSeparate" /></td>
				</tr>
			</table>
			</form>
		</div>
