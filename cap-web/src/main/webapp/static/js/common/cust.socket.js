...
//	    	var data = {"startTime":"xx:xx:xx","endTime":"oo:oo:oo","content":"測試內容12345678901234567890123456789012345678901234567890"};
	    	var message = "".concat("<span class='cal'><br/>"+i18n.def['remind.startDate']+data.startTime+"<br/>")
	    ...
	    socket.on('popUpEvent', function(data) {
	        ...
	        if (!_idd.size()) {
	        	var message = "".concat("<br/>"+i18n.def['remind.startDate']+data.startTime+"<br/>")
		    	.concat(i18n.def['remind.endDate']+data.endTime+"<br/>")
		    	.concat(i18n.def['remind.content']+data.content);
	            _idd = $("<div id='remindDialog' style='display:none'><form onsubmit='return false;'><table class='tb2' border='0' cellpadding='0' cellspacing='0' width='100%'><tbody><tr><td class='hd1' width='15%'>"+i18n.def['remind.date']+"</td><td>"+data.startTime+" ~ "+data.endTime+"</td></tr><tr><td>"+i18n.def['remind.item']+"</td><td class='text-breakAll'>"+message+"</td></tr></tbody></table></form></div>").appendTo("body");
	        }
	        _idd.dialog({
