pageInit(function(){
	$(document).ready(function(){
		...
		var grid = $("#gridview").jqGrid({
			url: 'webroot/remindhandler/queryDetail',
			height: "100", width: "100%",
			postData: { oid : oid },
            multiselect: true, hideMultiselect: true, autowidth: true, localFirst: true,
            colModel: [{
                header: i18n['remindPage']['styleTyp'],//"提醒方式",
                name: 'styleTyp', align: "left", width: 15, sortable: false
            },  {
                header: i18n['remindPage']['styleNum'],//"數值
                name: 'style', align: "left", width: 15, sortable: true
            },  {
                header: i18n['remindPage']['styleUnit'],//"單位
                name: 'unit', align: "left", width: 10, sortable: true
            },  {
		...
		if(oid){
			$.ajax({
		        url:"webroot/remindhandler/queryForm",
		        data:{
		        	oid: oid
		        },
		...
		var dDialog = $("#DETAIL").dialog({
			title:i18n['remindPage']["detail.title"],
        	...
        	buttons:API.createJSON([{
        		...
        		value:function(){
        			...
        			$.ajax({
    	                url: "webroot/remindhandler/saveDetail",
		...
		$(".btns").find("#add").click(function(){//新增
			if(oid){
				...
				API.showConfirmMessage(i18n.def.saveBeforeAction, function(data){
		            data && save({done:function(){dDialog.dialog('open')}});
		        });
			}
	        ...
	        if (ret) {
	            ...
	            API.showConfirmMessage(i18n.def['del.confrim'], function(b){
	                if (b) {
	                    $.ajax({
	                    	url: 'webroot/remindhandler/deleteDetail',
	                        data: {
	                            oids: oids
	                        },
		...
		var save = function(action){
			$form.validationEngine('validate') && 
			$.ajax({
                url: "webroot/remindhandler/save",
                data: $form.serializeData(),
                success: function(rtn){
                	$form.injectData({oid:rtn.oid});
