    ...
    $(document).ready(function(){
        var grid = $("#gridview").jqGrid({
            ...
            colModel: [{header: 'ID',
                ...
                name: 'JOB_NAME',width: 20
            }, {header: i18n['jobexecution']['startDate'],
                name: 'startDate',index:'START_TIME',width:10,align:'center',sortable:false
                ...
                name: 'jobInsId',index:'JOB_INSTANCE_ID',width:5,
        	...
        	buttons:API.createJSON([{
        		...
        		value:function(){
        			grid.jqGrid('setGridParam', {
                        postData: $.extend(qDialog.find("#qform").serializeData(),{jobInsId:''})
                    });
        ...
        $(".btns").find("#qry").click(function(){ //查詢
			...
			if (ret) {
				$.ajax({
	                ...
	                success: function(je){
	                	grid.jqGrid('setGridParam', {
	                        postData: {jobExeId:ret.JOB_EXECUTION_ID}
	                    });
	                    grid.trigger("reloadGrid");
	                }
	            });
			...
			if (ret) {
				$.ajax({
	                ...
	                success: function(je){
	                	grid.jqGrid('setGridParam', {
	                        postData: {jobInsId:ret.JOB_INSTANCE_ID}
	                    });
	                    grid.trigger("reloadGrid");
	                }
	            });
        ...
        var jdgrid = $("#jobDetailgrid").jqGrid({
            ...
            colModel: [{header: i18n['jobexecution']['step.name'],
                name: 'STEP_NAME',width: 15
            }, {header: i18n['jobexecution']['step.read'],
                name: 'READ_COUNT',width:5,formatter:'integer',align:'right'
            }, {header: i18n['jobexecution']['step.write'],
                name: 'WRITE_COUNT',width:5,formatter:'integer',align:'right'
            }, {header: i18n['jobexecution']['step.commit'],
                name: 'COMMIT_COUNT',width:5,formatter:'integer',align:'right'
            }, {header: i18n['jobexecution']['step.rollback'],
                name: 'ROLLBACK_COUNT',width:5,formatter:'integer',align:'right'
            }, {header: i18n['jobexecution']['duration'],
                name: 'duration',width: 10,align:'center'
            }, {header: i18n.def.status,
                name: 'EXIT_CODE',width: 10
